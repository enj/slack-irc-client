...
var SLACK_EVENTS = {
  ...
  message: function message(context, message) {
    ...
    if (target === '_slack_irc_stats_') {
      return;
    }

    context.logger.info('::CHAT:: to %s: %s', target, message.text);

    if (group && target === '_slack_irc_cmds_') {
      var irc_cmd = message.text.replace(/\\/g, "").split(/\s/).filter(function(s) {
        return s !== '';
      });
      if (irc_cmd.length) {
        irc_cmd[0] = irc_cmd[0].toUpperCase();
        context.irc.client.send(...irc_cmd);
      }
      context.logger.info('::IRC_CMD:: [%s]', irc_cmd);
      return;
    }
