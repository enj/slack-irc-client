...
var HELPERS = {
  printData: function printData(data) {
    var cache = [];
    var out = JSON.stringify(data, function(key, value) {
  ...
  sendTarget: function sendTarget(context, target, targetName, from, text, options, targetType) {
    ...
    if (text.match(MENTIONS)) {
      var mentionedData = "{0} -> {1}: {2}\nData: {3}".format(from, targetName, text, HELPERS.printData(target));
      HELPERS.sendUser(context, 'You were mentioned', mentionedData);
      text += DIRECT_MENTION;
    }
