...
var MENTIONS = /(\bmo\b|\bmkhan\b|\benj\b|\bmonis\b)/i;
var DIRECT_MENTION = ' @everyone';
...
var HELPERS = {
  handleNotice: function handleNotice(context, from, to, text, description) {
    if (to != undefined && HELPERS.isIrcChannel(context, to)) {
      HELPERS.sendChannelStatus(context, to, "{0} from {1}: {2}".format(description, from, text));
    } else {
      context.logger.info('::NOTICE:: [%s] %s -> %s: %s', description, from, to, text);
    }
  },
  ...
  sendTarget: function sendTarget(context, target, targetName, from, text, options, targetType) {
    ...
    var publicMessage = from !== targetName;
    var channelInfo = publicMessage ? ' on #' + targetName : '';
    var chatData = "{0}{1}: {2}".format(from, channelInfo, text);
    context.logger.info('::CHAT:: ' + chatData);

    if (publicMessage && text.match(MENTIONS)) {
      HELPERS.sendUser(context, 'You were mentioned', chatData);
      text += DIRECT_MENTION;
    }
    ...
    if (_.isArray(opts.attachments)) {
      opts.attachments = JSON.stringify(opts.attachments, null, 2);
    }
...
var IRC_EVENTS = {
  ...
  quit: function quit(context, nick, reason, channels, message) {
    ...
    //   }
    // });
    HELPERS.sendChannelStatus(context, '_slack_irc_stats_', nick + ' quit: ' + reason);
  },
  ...
  kill: function kill(context, nick, reason, channels, message) {
    ...
    //   }
    // });
    HELPERS.sendChannelStatus(context, '_slack_irc_stats_', nick + ' killed: ' + reason);
  },
  ...
  notice: function notice(context, from, to, text, message) {
    context.logger.verbose('IRC: New notice from %s: %s', from, text);
    HELPERS.handleNotice(context, from, to, text, 'Notice');
  },
  'ctcp-notice': function ctcpVersion(context, from, to, text, message) {
    ...
    HELPERS.handleNotice(context, from, to, text, 'CTCP notice');
  },
  'ctcp-privmsg': function ctcpVersion(context, from, to, text, message) {
    ...
    HELPERS.handleNotice(context, from, to, text, 'CTCP privmsg');
  },
  ...
  nick: function nick(context, oldnick, newnick, channels, message) {
    ...
    //   }
    // });
    HELPERS.sendChannelStatus(context, '_slack_irc_stats_', oldnick + ' changed nick to ' + newnick);
  },
  ...
  error: function error(context, message) {
    ...
    HELPERS.sendUser(context, 'IRC Error', JSON.stringify(message, null, 2));
  }
};
